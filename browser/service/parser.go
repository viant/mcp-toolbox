...
import (
	...
	smatcher "github.com/viant/mcp-toolbox/browser/service/matcher"
	...
	for {
		...
		case selector:
			...
			case method:
				...
				if len(callParams) > 0 {
					call.Parameters = []any{callParams}
				}
			case id:
...
func newIllegalTokenParsingError(index, token int, expected string) error {
	return fmt.Errorf("illegal token:%v at %v, expected %v", token, index, expected)
}
